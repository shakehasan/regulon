...
| `docs/GLOSSARY.md` | Plain-language definitions of every term used in the repo | Add an entry whenever you introduce a new term of art |
| `.github/badges/coverage.json` | Real coverage number, shields.io endpoint schema | Regenerate with `make badge-coverage` — never hand-edit |
...
make ci       # lint + type + test + safety, in CI's order — the full local gate
make badge-coverage  # regenerate .github/badges/coverage.json from a real test run
...
- The README coverage badge is the same rule applied to a badge: `.github/badges/coverage.json`
  is generated by `make badge-coverage` from an actual `coverage report`, never typed by hand.
