...
def isolated_env(monkeypatch, tmp_path):
    """Point settings at a temp YAML file and clear QUORUM_ env overrides."""
    for var in ("QUORUM_APP_NAME", "QUORUM_ENVIRONMENT", "QUORUM_DATA_DIR"):
        monkeypatch.delenv(var, raising=False)
    config_file = tmp_path / "quorum.yaml"
    monkeypatch.setenv("QUORUM_CONFIG_FILE", str(config_file))
    return config_file
...
def test_env_overrides_yaml(isolated_env, monkeypatch):
    isolated_env.write_text("environment: ci\n", encoding="utf-8")
    monkeypatch.setenv("QUORUM_ENVIRONMENT", "prod")
    settings = load_settings()
...
def test_repo_config_file_loads():
    """The committed config/quorum.yaml parses into valid settings."""
    repo_root = Path(__file__).resolve().parents[2]
    assert (repo_root / "config" / "quorum.yaml").is_file()
    settings = Settings()
    assert settings.app_name == "quorum"
