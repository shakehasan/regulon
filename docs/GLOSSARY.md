...
`parse → normalize → redact → chunk → store`. Regulon's runs in
[`src/regulon/ingestion/`](../src/regulon/ingestion/).
...
half-open (the start character is included, the end character is not). Regulon guarantees
...
once. Regulon's ingest is idempotent: because ids are content hashes, re-ingesting a corpus
...
default a single SQLite file (`data/regulon.sqlite3`), so there is no server to run; pgvector is an
...
phone numbers, government ID numbers such as the US SSN shape `NNN-NN-NNNN`. Regulon strips it at
...
Public-domain, information-dense, and free to fetch, which is why it is one of Regulon's two data
sources.

**10-K** — a company's annual report to the SEC: business overview, risk factors, management
discussion, and audited financials. The document type Regulon's demo workflow reads.
...
fenced by `---` lines. Regulon's synthetic documents declare their title, ticker, form, fiscal
year, and synthetic status there.

**Synthetic corpus** — documents Regulon generates rather than collects: fictional companies,
